...
public class Dijkstra {
  ...
  public void doDijkstra(String s) {
        ...
        while (!list.isEmpty()){
            ...
            for (Edge edge: smallest.adjacentEdges){
                Vertex w = edge.target;
                if (!w.known){
                    Double cvw = edge.distance;
                    if (smallest.distance + cvw < w.distance){
                        w.distance = smallest.distance + cvw;
                        w.prev = smallest;
                        list.add(w);
                    }
                }
  ...
  public List<Edge> getDijkstraPath(String s, String t) {
    ...
    for (Vertex v = getVertex(t); v.prev != null; v = v.prev){
        path.addFirst(new Edge(v.prev, v, computeEuclideanDistance(v.prev.x, v.prev.y, v.x, v.y)));
    }
    return path;
  }
